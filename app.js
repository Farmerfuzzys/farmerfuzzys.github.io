...
const STORAGE_RESET_KEY = "fire_dispatcher_reset_v6";
...
function ensureSeedData() {
  ...
  localStorage.removeItem(ROLES_STORAGE_KEY);  // Clear any old roles
...
function getStoredRoles() {
  return readJson(ROLES_STORAGE_KEY, []);
}

export function setRoles(roles) {
  writeJson(ROLES_STORAGE_KEY, normalizeRoles(roles));
}
...
// Demo login removed - use real Firebase authentication

export function requireAnyRole(allowedRoles) {
  const roles = getStoredRoles();
  if (!roles || !roles.length) {
    return false;  // No roles = not authenticated
  }
...
export function enforcePageAccess(allowedRoles) {
  ...
  if (!user || !user.id || !roles || !roles.length) {
    window.location.href = "index.html";
    return false;
  }
